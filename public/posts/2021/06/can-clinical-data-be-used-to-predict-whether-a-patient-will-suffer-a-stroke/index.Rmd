...
```{r, echo = F, message=F, warning=F, fig.width=7, fig.height=7.5, fig.align='center'}
long_cat_data <- data %>%
...
```{r, message=F, warning=F, fig.align='center', fig.width=7}
long_num_data <- data %>%
...
final %>%
  pluck(".workflow", 1) %>%   
  pull_workflow_fit() %>% 
  vip(num_features = 20, aesthetics = list(col = "navyblue", fill = "navyblue"))
